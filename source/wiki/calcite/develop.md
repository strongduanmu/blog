...
您可以通过[下载 Release 版本](https://calcite.apache.org/downloads)或从源代码控制获取源代码。
...
[请使用ASF 的自助服务设施](https://selfserve.apache.org/jira-account.html)申请帐户。

## 贡献

我们欢迎贡献。

如果您打算做出重大贡献，请先与我们联系！它有助于就总体方法达成一致。为您提议的功能记录 [JIRA 案例](https://issues.apache.org/jira/browse/CALCITE)或在开发列表上开始讨论。

在打开新的 JIRA 案例之前，请查看现有问题。您计划处理的功能或错误可能已经存在。

如果需要创建一个新问题，提供简洁且有意义的摘要行非常重要。它应该暗示最终用户试图做什么、在哪个组件中以及看到了什么症状。如果不清楚所需的行为是什么，请改写：例如，“验证程序关闭模型文件”为“验证程序不应关闭模型文件”。

该案例的贡献者应随时重新措辞并澄清摘要内容。如果您在澄清时删除了信息，请将其放入案例描述中。
...
如果您要立即处理该问题，请立即将其分配给您自己。要将问题分配给自己，您必须在 JIRA 中注册为贡献者。为此，请按照 [JIRA 帐户](https://calcite.apache.org/develop/#jira-accounts)部分中概述的说明进行操作。

如果您致力于在即将发布的版本之前修复问题，请相应地设置修复版本（例如 1.20.0），否则将其留空。

如果您发现现有问题，请将其标记为“正在进行”，并在完成后将其标记为“pull-request-available”。

如果出于任何原因您决定某个问题不能进入正在进行的版本，请将修复版本重置为空白。
...
Fork GitHub 存储库，并为您的功能创建一个分支。

开发您的功能和测试用例，并确保 `./gradlew build`成功。 （如果您的更改需要的话，请运行额外的测试。）

将更改提交到您的分支，并使用以 JIRA 案例编号开头的注释，如下所示：

```
[CALCITE-345] AssertionError in RexToLixTranslator comparing to date literal
```

如果您的更改有多个提交，请使用`git rebase -i main`将它们压缩为单个提交，并使您的代码与主线上的最新代码保持同步。

为了保持提交历史记录的干净和统一，您应该遵守以下准则。
...
- 以大写字母开头。
- 不要以句号结束。
- 使用祈使语气（“添加处理程序……”）而不是过去时（“添加处理程序……”）或现在时（“添加处理程序……”）。
- 如果可能，请描述您更改的用户可见行为（“FooCommand 现在创建目录，如果它不存在”），而不是实现（“为 FileNotFound 添加处理程序”）。
- 如果您正在修复错误，那么描述该错误就足够了（“如果用户未知，则出现 NullPointerException”），人们会正确地推测您的更改的目的是修复该错误。

然后将您的提交推送到 GitHub，并创建从您的分支到方解石主分支的拉取请求。更新 JIRA 案例以引用您的拉取请求，提交者将审查您的更改。

拉取请求可能需要更新（提交后），主要原因有以下三个：

1. 您在提交拉取请求后发现了问题；
2. 审稿人要求进一步修改；
3. CI 构建失败，并且失败不是由您的更改引起的。

为了更新拉取请求，您需要在分支中提交更改，然后将提交推送到 GitHub。我们鼓励您在以前现有的提交之上使用常规（非重新基址）提交。

将更改推送到 GitHub 时，您应该避免使用该`--force` 参数及其替代方案。您可以选择在某些条件下强制推送更改：

- 拉取请求是在不到 10 分钟前提交的，并且没有与之相关的待讨论（在 PR 和/或 JIRA 中）；
- 审阅者明确要求您执行一些需要使用该`--force`选项的修改。

在特殊情况下，CI 构建失败，并且失败不是由您的更改引起的，创建一个空提交 ( `git commit --allow-empty`) 并推送它。

## 空安全

Apache Calcite 使用 Checker 框架来避免意外的情况`NullPointerExceptions`。您可以在 https://checkerframework.org/ 找到详细的文档
...
- 默认情况下，参数、返回值和字段不可为空，因此请不要使用`@NonNull`

- 局部变量从表达式推断为空，因此您可以编写`Object v = ...`而不是`@Nullable Object v = ...`

- 避免使用`javax.annotation.*`注释。来自的注释`jsr305`不支持类似的情况`List<@Nullable String>` ，因此最好坚持使用`org.checkerframework.checker.nullness.qual.Nullable`.不幸的是，Guava（截至`29-jre`）同时具有 和 依赖项，因此您可能需要配置您的 IDE 以**从** `jsr305`代码完成中排除注释。`checker-qual``javax.annotation.*`

- Checker 框架逐个验证代码。这意味着，它无法解释方法执行顺序。这就是为什么`@Nullable`应该在使用字段的每个方法中验证字段的原因。如果将逻辑拆分为多个方法，您可能需要验证一次 null，然后通过不可为 null 的参数传递它。对于以 null 开头并随后变为非 null 的字段，请使用`@MonotonicNonNull`.对于已检查是否为 null 的字段，请使用`@RequiresNonNull`.

- 如果您绝对确定该值不为空，则可以使用`org.apache.calcite.linq4j.Nullness.castNonNull(T)`.背后的意图`castNonNull`就像`trustMeThisIsNeverNullHoweverTheVerifierCantTellYet(...)`

- 但是，如果表达式可为空，则需要将其传递给非空方法，请使用`Objects.requireNonNull`。它允许获得包含上下文信息的更好的错误消息。

- Checker Framework 附带了带注释的 JDK，但是，可能存在无效注释。在这种情况下，可以放置存根文件`/src/main/config/checkerframework`来覆盖注释。文件具有扩展名非常重要，`.astub`否则它们将被忽略。
...
- 但是，默认边界是不可为空的，因此如果您编写`<T extends Number>`，则它与 相同`<T extends @NonNull Number>`。
...
- 如果您需要“可为空或不可为空`Number`”，则使用`<T extends @Nullable Number>`，

- 如果需要确保类型**始终**可为空，请`<@Nullable T>`按如下方式使用：
...
Calcite 利用[GitHub 操作](https://github.com/apache/calcite/actions?query=branch%3Amain)进行持续集成测试。

## 入门

Calcite 是一个社区，因此加入该项目的第一步是自我介绍。加入[开发人员列表](https://mail-archives.apache.org/mod_mbox/calcite-dev/) 并发送电子邮件。

如果您有机会参加[聚会](https://www.meetup.com/Apache-Calcite/)，或 在会议上与[社区成员](https://calcite.apache.org/develop/#project-members)见面，那也很棒。

选择要执行的初始任务。它应该是非常简单的事情，例如错误修复或[我们标记为“新手”的 Jira 任务](https://issues.apache.org/jira/issues/?jql=labels %3D newbie %26 project %3D Calcite %26 status %3D Open)。请遵循[贡献指南](https://calcite.apache.org/develop/#contributing)来承诺您的更改。

我们重视所有有助于建立充满活力的社区的贡献，而不仅仅是代码。您可以通过测试代码、帮助验证版本、编写文档、改进网站或仅回答列表中的问题来做出贡献。

在您做出一些有用的贡献后，我们可能会邀请您成为[提交者](https://community.apache.org/contributors/)。
