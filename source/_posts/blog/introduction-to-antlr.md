...
```bash
# 第一种：java -jar 直接运行 ANTLR 的 jar 包
java -jar /usr/local/lib/antlr-4.8-complete.jar
# 第二种：直接调用 org.antlr.v4.Tool 类
...
```bash
# 使用 Hello 语法和 r 规则启动 TestRig
grun HelloWorld r -tokens
# 输入要识别的语句
hello world
# 输入回车符结束输入（Mac 下输入 Crtl + D，Win 下输入 Ctrl + Z）
...
![Antrl GUI 预览](introduction-to-antlr/antlr-gui-preview.png)
...
![语法分析过程](introduction-to-antlr/language-recognizer.png)
...
![语法分析树](introduction-to-antlr/sytax-tree.png)
...
![ParseTreeWalker 深度优先遍历](introduction-to-antlr/sytax-tree-listener.png)
...
![ParseTreeListener 调用完整流程](introduction-to-antlr/sytax-tree-listener-process.png)
...
![访问器遍历过程](introduction-to-antlr/sytax-tree-visitor.png)
...
![安装 IDEA ANTLR 插件](introduction-to-antlr/intstall-idea-plugin.png)
...
![使用 IDEA 插件预览](introduction-to-antlr/using-idea-plugin-preview.png)
...
![Maven ANTLR 插件](introduction-to-antlr/maven-antlr-plugin.png)
