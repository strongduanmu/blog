...
执行 `java -jar sqlancer-*.jar -h` 命令可以查看 SQLancer 所有命令参数及说明，下面展示的内容进行了一些截取，重点展示 SQLancer 全局选项（`options`） 以及 MySQL 数据库的参数选项。从 Usage 格式可以看出，SQLancer 命令需要先指定全局选项 `options`（例如：`--num-threads 4`），然后再指定数据库专属命令 `command`（例如：`sqlite3`） 以及数据库专属选项 `command options`（例如：`--oracle NoREC`）。
...
Usage: SQLancer [options] [command] [command options]
  Options:
    ...
    --storage-unit-port
      ...
      Default: 3306
    --storage-unit-user
      Storage unit user for ShardingSphere
      Default: root
