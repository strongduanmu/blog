---
title: MySQL 5.7.36 安装及配置指南
tags: [MySQL]
categories: [MySQL]
date: 2021-12-25 19:23:13
...
```bash
wget https://cdn.mysql.com//Downloads/MySQL-5.7/mysql-5.7.36-linux-glibc2.12-x86_64.tar.gz --no-check-certificate
```
...
# 创建数据目录
mkdir /data/{mysql,mysql-log} -p
# 对用户进行赋权
chown -R mysql:mysql mysql-5.7.25/
chown -R mysql:mysql /data/mysql*
```

初始化MySQL，如果`datadir`中包含文件，则会报一下错，删除该目录下文件后重试：

```bash
./bin/mysqld --initialize --user=mysql --basedir=/opt/mysql/mysql-5.7.25/  --datadir=/data/mysql
```

![mysql-initialize-erro](../../../../../../images/articles/mysql-initialize-error.png)

重试之后，生成了初始密码（`uq!io/,xp3xQ`）：

![mysql-install-password](../../../../../../images/articles/mysql-install-password.png)
...
[client]
no-beep
socket =/opt/mysql/mysql-5.7.25/mysql.sock
# pipe
# socket=0.0
port=3306
[mysql]
...
datadir=/data/mysql
port=3306
pid-file=/opt/mysql/mysql-5.7.25/mysqld.pid
#skip-grant-tables
skip-name-resolve
socket = /opt/mysql/mysql-5.7.25/mysql.sock
...
需要修改启动脚本，修改`basedir`、`bindir`、`datadir`、`sbindir`、`libexecdir`地址：

```bash
if test -z "$basedir"
then
  basedir=/opt/mysql/mysql-5.7.25
  bindir=/opt/mysql/mysql-5.7.25/bin
  if test -z "$datadir"
  then
    datadir=/data/mysql
  fi
  sbindir=/opt/mysql/mysql-5.7.25/bin
  libexecdir=/opt/mysql/mysql-5.7.25/bin
else
  bindir="$basedir/bin"
  if test -z "$datadir"
  then
...
然后按照前文RPM安装方式中的方法，重置密码。

## MySQL配置

MySQL从5.7.18开始，不在二进制包中提供my-default.cnf文件，参考[官网文章](https://dev.mysql.com/doc/refman/5.7/en/binary-installation.html)。参考博客[MySQL的my.cnf文件](https://www.cnblogs.com/EasonJim/p/7158466.html)，获得了一个比较全的配置示例：
...
以上配置文件中的配置项，需要根据服务器的实际情况进行添加，仅供学习参考。
